...
测试：mpif90 -v  
(2) 之前如果有串行编译过，则需要清除编译文件，仅保留源文件，命令为：make clean  
配置：./configure 或 ./configure -enable-parallel ，提示配置成功  
(3) 编译所有：make all （仅编译pw：make pw）  
再编译epw：make epw  
编译wannier90报错，可能是wannier90第三方安装包无法下载或者下载不完全，解决方法：可本地手动下载wannier90源码，然后改名并上传到Quantum ESPRESSO根目录下arch目录下，再重新编译！  
(4) 测试（可选，非必须）
进入文件夹：cd test-suite/  
...
(6) 添加环境变量：  
1) 方法一（推荐）：编辑 .bashrc 文件，添加内容：export PATH=/opt/qe-7.3.1/bin:$PATH ，保存后输入命令更新：source .bashrc 。  
